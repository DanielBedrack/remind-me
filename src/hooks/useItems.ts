...
export function useItems(userId: string | null) {
  ...
  async function add(name: string, quantity: number, storeType: StoreType, storeName?: string) {
    ...
    logHistory({ action: 'added', itemName: name, storeType, storeName, qty: quantity });
  }
  ...
  async function update(itemId: string, name: string, quantity: number, storeType: StoreType, storeName?: string) {
    ...
    logHistory({ action: 'updated', itemName: name, storeType, storeName, qty: quantity });
  }
  ...
  async function updateQuantity(itemId: string, quantity: number) {
    ...
    if (item) logHistory({ action: 'updated', itemName: item.name, storeType: item.storeType, storeName: item.storeName, qty: quantity });
  }

  async function remove(itemId: string) {
    const item = items.find((i) => i.id === itemId);
    await removeItem(itemId);
    if (item) logHistory({ action: 'deleted', itemName: item.name, storeType: item.storeType, storeName: item.storeName, qty: item.quantity });
  }

  async function collect(itemId: string) {
    const item = items.find((i) => i.id === itemId);
    await removeItem(itemId);
    if (item) logHistory({ action: 'collected', itemName: item.name, storeType: item.storeType, storeName: item.storeName, qty: item.quantity });
  }
