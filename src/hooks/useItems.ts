...
import { addItem, removeItem, subscribeToItems } from '../services/firebase';
...
export function useItems(userId: string | null) {
  ...
  useEffect(() => {
    ...
    const unsub = subscribeToItems(userId, async (updated) => {
      setItems(updated);
      setLoading(false);
      // Keep a local cache for the background task
  ...
  async function add(name: string, quantity: number, storeType: StoreType) {
    if (!userId) throw new Error('Not signed in yet — please wait a moment and try again.');
    await addItem(userId, { name, quantity, storeType });
  }
